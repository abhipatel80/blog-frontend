                ...
                return (
                    <div className='justify-items-center grid my-10' key={val._id}>
                        <div className='shadow-md bg-white md:w-4/6 w-full rounded-md p-7'>
                            <h1 className='md:text-2xl text-lg font-semibold pb-4 flex'>{val.title}
                                {location.pathname === "/myblog" ? <span className='ml-auto'>
                                    <button onClick={() => editBlog(val._id)}>
                                        <i className="fa-solid fa-pen-to-square mr-7 hover:text-green-700 cursor-pointer"></i>
                                    </button>
                                    <button onClick={() => dispatch(deleteBlogAsync({ id: val._id, userId: val.userId }))}>
                                        <i className="fa-solid fa-trash cursor-pointer hover:text-red-600"></i>
                                    </button>
                                </span> : ""}
                            </h1>
                            <h2 className='pb-4 text-sm md:text-lg text-gray-500'>{val.userName} <span className='pl-1 text-2xl'> · </span> {dateFormat(val.createdAt)}</h2>
                            <p className='text-sm md:text-lg'>{parse(val.description.slice(0, 300))}</p>
                            <NavLink to={`/${val._id}`}>
                                <button className='py-2 px-6 text-sm text-white hover:bg-purple-700 mt-5 bg-purple-600 rounded-md'>Read More</button>
                            </NavLink>
                        </div>
                    </div>
                )
